...
*
*              stk_size  is the size of the stack in number of elements.  If OS_STK is set to uint8_t,
...
//    if (((uint32_t)stk_start >= TASK_STACK_USING_MEM_UPPER_RANGE)
//		||(((uint32_t)stk_start + stk_size) >= TASK_STACK_USING_MEM_UPPER_RANGE))
//    {
...
uint8_t tls_os_task_schedule_state()
{
   return (uint8_t)xTaskGetSchedulerState();
}
...
*/
 ...
 tls_os_status_t tls_os_mutex_create(uint8_t prio,
...
uint16_t tls_os_sem_get_count(tls_os_sem_t *sem)
{
    return (uint16_t)xSemaphoreGetCount((xQUEUE *)sem);
}
 ...
 tls_os_status_t tls_os_queue_delete(tls_os_queue_t *queue)
{
	...
	if ((uint32_t *)(((xQUEUE *)queue)->pcHead) >= &__heap_start)		//如果没有从堆申请，不用释放)
	{
...
uint8_t tls_os_queue_is_empty(tls_os_queue_t *queue)
{
...
tls_os_status_t tls_os_queue_remove(tls_os_queue_t *queue, void* msg, uint32_t msg_size)
{
    void *tmp_ev;
    BaseType_t ret;
    int i;
...
uint8_t tls_os_timer_active(tls_os_timer_t *timer)
{
    return (uint8_t)xTimerIsTimerActive((xTIMER *)timer);
}
uint32_t tls_os_timer_expirytime(tls_os_timer_t *timer)
{
    return (uint32_t)xTimerGetExpiryTime((xTIMER *)timer);
}
