...
bool DRV8301_Config(RegisterFile *pRegs, float requested_gain, float *actual_gain)
{
	...
	int16_t regs_equal = (pRegs->control_register_1 == new_config.control_register_1) && (pRegs->control_register_2 == new_config.control_register_2);
...
int16_t DRV8301_Init(RegisterFile *pRegs)
{
	...
	int16_t wrote_regs = DRV8301_WriteReg(kRegNameControl1, pRegs->control_register_1) + DRV8301_WriteReg(kRegNameControl1, pRegs->control_register_1) + DRV8301_WriteReg(kRegNameControl1, pRegs->control_register_1) + DRV8301_WriteReg(kRegNameControl1, pRegs->control_register_1) + DRV8301_WriteReg(kRegNameControl1, pRegs->control_register_1) + DRV8301_WriteReg(kRegNameControl2, pRegs->control_register_2);
	if (wrote_regs != 0)
	{
		return -1;
	}
